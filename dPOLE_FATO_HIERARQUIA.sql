let
    ...
    #"Coluna condicional inserida 1" = Table.AddColumn(#"Valor substituído 1", "N1 NewFEV24", each if [N3] = "001200010000000001" then "'00080002" else if [N3] = "001200010000000005" then "'00080002" else if [N3] = "001200010000000008" then "'00080005" else if [N3] = "001200010000000011" then "'00080002" else if [N3] = "001200010000000004" then "'00080002" else if [N3] = "001200010000000006" then "'00080002" else if [N3] = "001200010000000009" then "'00080002" else if [N3] = "001200010000000012" then "'00080002" else if [N3] = "001200010000000013" then "'00080002" else if [N3] = "001200010000000015" then "'00080002" else if [N3] = "001200010000000014" then "'00080002" else if [N3] = "001200010000000000" then "'00080002" else if [N3] = "001200010000000016" then "'00080002" else if [N3] = "001200010000000017" then "00080002" else if [N3] = "001200010000000008" then "00080005" else [N1 Comercial] ),
    #"Coluna condicional inserida 2" = Table.AddColumn(#"Coluna condicional inserida 1", "N2 NewFEV24", each if [N3] = "001200010000000004" then "'000800020010" else if [N3] = "001200010000000006" then "000800020009" else if [N3] = "001200010000000009" then "'000800020010" else if [N3] = "001200010000000012" then "'000800020010" else if [N3] = "001200010000000013" then "'000800020009" else if [N3] = "001200010000000008" then "'000800050013" else if [N3] = "001200010000000015" then "'000800020008" else if [N3] = "001200010000000016" then "'000800020010" else if [N3] = "001200010000000017" then "000800020010" else if [N3] = "001200010000000017" then "000800050013" else [N2]),
    #"Transformar colunas" = Table.TransformColumnTypes(#"Coluna condicional inserida 2", {{"N1 NewFEV24", type text}, {"N2 NewFEV24", type text}}),
    #"Colunas Renomeadas" = Table.RenameColumns(#"Transformar colunas",{{"N1", "N1 - M"}, {"N2", "N2 - M"}, {"N3", "N3 - M"}}),
    #"Últimos caracteres extraídos" = Table.TransformColumns(#"Colunas Renomeadas", {{"N1 Comercial", each Text.End(_, 8), type text}}),
    #"Últimos caracteres extraídos1" = Table.TransformColumns(#"Últimos caracteres extraídos", {{"N1 NewFEV24", each Text.End(_, 8), type text}}),
    #"Últimos caracteres extraídos2" = Table.TransformColumns(#"Últimos caracteres extraídos1", {{"N2 NewFEV24", each Text.End(_, 12), type text}})
in
    #"Últimos caracteres extraídos2"
#---------------------------------------------------------------------------------------------------------------------
